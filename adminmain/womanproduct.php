...
<html lang="en">
    <style>
        #wform{width:60%;margin:10px 80px;padding:10px;border:1px solid #ffdead;border-radius:10px;background:silver;}
        #wform input{margin-bottom:10px;}
        </style>
    <head>
        <title></title>
        <meta charset="UTF-8">
    ...
    <body>
        <h2 class="text-center"style="background:bisque;">Add WOMAN PRODUCTS</h2><hr style="border:1px solid bisque;">
    <form id="wform"action="womanproduct.php" method="POST" enctype="multipart/form-data">
...
if(isset($_POST["btn"]))
{
    ...
    include "../connect.php";
    $cmd="Insert into tbl_woman(prodname,prodstyle,photo,detail,category,price,date) values('$name','$styno','$destpath','$desc','$catg','$price','$dt')";
    $x=MySQLi_Query($con,$cmd);
    if($x)
        echo "<script>alert('woman product added successfully.');window.location.href='womanproduct.php';</script>";
    else
        echo "<script>alert('Unable to save woman product details..');window.location.href='womanproduct.php';</script>";
